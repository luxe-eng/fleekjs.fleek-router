...
var router = function(app, _config) {
  ...
  _.each(swagger.sanitizedRoutes, function(routeObj) {
    ...
    if (!_.isFunction(methodHandler)) { throw new Error('Method [' + routeObj.method.toUpperCase() + '] does not exist for controller: ' + routeObj.controller); }

    function* bindRouteData (next) {
      if(!this.fleek) this.fleek                         = {};
      if(!this.fleek.controllers) this.fleek.controllers = swagger.controllers;
    ...
    if (routeObj.authRequired) {
      secureRouter[routeObj.method](path, bindRouteData, fleekResponse, authenticate, validate, middleware, methodHandler);
    } else {
      publicRouter[routeObj.method](path, bindRouteData, fleekResponse, validate, middleware, methodHandler);
    }
  });
