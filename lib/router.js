...
var injector      = require('swagger-injector');
...
var router = function(app, _config) {
  ...
  if (config.injector) {
    config.injector = _.isObject(config.injector) ?  config.injector : {};
    config.injector.swagger = config.injector.swagger || docs;
    app.use(injector.koa(config.injector));
  }
